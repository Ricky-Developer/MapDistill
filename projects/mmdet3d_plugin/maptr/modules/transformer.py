...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        if lss_encoder is not None:
            self.lss_encoder = build_transformer_layer_sequence(lss_encoder)
    ...
    def lss_bev_encode_student(
        ...
        images = mlvl_feats[0]
        img_metas = kwargs['img_metas']
        bev_embed = self.lss_encoder(images,img_metas)
        bs, c, _,_ = bev_embed.shape
        bev_embed = bev_embed.view(bs,c,-1).permute(0,2,1).contiguous()
        return bev_embed
    ...
    def get_bev_features(
        ...
        if lidar_feat is None:
            # student
            # lss_bev_embed: [bs, 20000, 256]
            lss_bev_embed = self.lss_bev_encode_student(
                mlvl_feats,
                prev_bev=prev_bev,
                **kwargs)
            bs = mlvl_feats[0].size(0)
            tmp_bev_embed = bev_embed.view(bs, bev_h, bev_w, -1).permute(0,3,1,2).contiguous()
            tmp_lss_bev_embed = lss_bev_embed.view(bs, bev_h, bev_w, -1).permute(0,3,1,2).contiguous()
            fused_bev_embed = self.fuser([tmp_bev_embed, tmp_lss_bev_embed])
            fused_bev_embed = fused_bev_embed.flatten(2).permute(0,2,1).contiguous()
            # cam_gkt_embed, cam_gkt_embed, lidar, cam_lss_embed, cam_lss_gkt_fused_embed
            return bev_embed, cam_embed, None, lss_bev_embed, fused_bev_embed
        else:
            bs = mlvl_feats[0].size(0)
            # bev_embed: [bs, 20000, 256] -> [bs, 256, 200, 100]
            ...
            # fused_bev: [bs, 20000, 256]
            # lidar_cam_fused_embed, cam_gkt_embed, lidar_embed, none, none
            return fused_bev, cam_embed, lidar_feat, None, None
    ...
    def forward(self,
        ...
        bev_embed, cam_embed, lidar_embed, lss_bev_embed, fused_bev_embed = self.get_bev_features(
            mlvl_feats,
            lidar_feat,
            bev_queries,
            bev_h,
        ...
        return bev_embed, inter_states, init_reference_out, inter_references_out, cam_embed, lidar_embed, lss_bev_embed, fused_bev_embed
