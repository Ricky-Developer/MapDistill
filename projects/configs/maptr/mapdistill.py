...
dbound=[1.0, 35.0, 0.5]
point_cloud_range_lss = [-6.0, -12.0,-10.0, 6.0, 12.0, 10.0] # 统一lss encoder 输出维度
voxel_size_lss = [0.15, 0.15, 20.0] # 统一lss encoder 输出维度
    ...
    pts_bbox_head_student=dict(
        ...
        transformer=dict(
            ...
            lss_encoder=dict(
                ...
                out_channels=_dim_,
                feat_down_sample=32,
                pc_range=point_cloud_range_lss,
        ...
        loss_pts=dict(type='PtsL1Loss', 
                      loss_weight=5.0),
        loss_dir=dict(type='PtsDirCosLoss', loss_weight=0.005),
        #source baseline low_high config
        #loss_low_level_bev=dict(type='L2Loss', loss_weight=1.0),
        #loss_high_level_bev=dict(type='L2Loss', loss_weight=1.0),
        loss_low_level_bev=dict(type='L2Loss', loss_weight=1.0),
        loss_high_level_bev=dict(type='L2Loss', loss_weight=1.0),
        loss_out_class=dict(
            type='KLLoss',
            ...
            reduction = 'batchmean',
            loss_weight=0.1),
        loss_out_pts=dict(type='PtsL1Loss', loss_weight=0.1),
        loss_relation=dict(
            type='L1Loss',
            loss_weight=1.0)
        ),
...
data_root = '/root/data1/PublicDataset/nuscenes/data/'
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'img_backbone_student': dict(lr_mult=0.1),
        }),
    weight_decay=0.01)
